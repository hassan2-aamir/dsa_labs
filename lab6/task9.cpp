...
int countInternalNodes(BST_Node* root){
    ...
    else if(root->LeftChild != nullptr or root->RightChild != nullptr) return 1 + countInternalNodes(root->LeftChild)+countInternalNodes(root->RightChild);    
    else return 0;
}
...
int main() {
    ...
    cout<<endl<<"Internal Nodes: "<<countInternalNodes(bst.root)<<endl;
